...
export interface CardProps extends ContentProps {
  useGB: boolean;
  onReset?: () => void;
}
...
const ContentTree: React.FC<ContentProps> = ({ devices = [], onRemove }) => {
  ...
  const prefixCls = getPrefixCls('device-selection-card');
...
export const Card: React.FC<CardProps> = ({ useGB, devices = [], onRemove, onReset }) => {
  const { getPrefixCls } = useContext(ConfigContext);
